...
The last two examples are, in particular, compatible with representations of $G$ over $\C$. This will prove to be important when we define the character of a representation.

Before doing so, however, we will need to outline the structure of the inner-product space of central functions on $G$.
...
\begin{proposition}[$\C$-Vector Space Structure of $\Fof{G, \C}$ and $\Fcof{G, \C}$] \hfill
    \begin{enumerate}[label = \normalfont \arabic*., noitemsep]
        \item $\Fof{G, \C}$ is a $\C$-vector space of dimension $\abs{G}$, with basis $\set{\delta_g : g \in G}$.
        \item $\Fcof{G, \C}$ is a subspace of $\Fof{G, \C}$ of dimension equal to the number of conjugacy classes of $G$, with basis $\set{\delta_g : g \text{ uniquely represents a conjugacy class of $G$}}$.
    \end{enumerate}
\end{proposition}

We do not prove this proposition here, as it merely involves checking a few conditions that follow directly from the definitions.
