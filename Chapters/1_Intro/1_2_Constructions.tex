...
\begin{boxexample}
    ...
    \end{enumerate}
\end{boxexample}
The $K = \F_2$ case in the above example demonstrates an important fact: \textit{the converse of Proposition \ref{Ch1:Prop:Irred_implies_Indecomp} is not, in general, true.} It is natural to wonder if we can, at least, prove a \textit{partial} converse. It turns out that we can: the converse is true under (indeed, equivalent to) a very specific condition. While we do not yet have the tools we need to prove this result, it will be worth the wait, as it will turn out to be a direct consequence of a more powerful theorem proved by Heinrich Maschke (and the converse thereof).
...
\end{boxdefinition}
Note that by this definition, any irreducible representation is completely reducible---it is a direct sum of only one irreducible representation, namely, itself.

Indecomposability and complete reducibility are intrinsically connected. The following proposition will eventually be part of a chain of equivalences that will establish a converse of Proposition \ref{Ch1:Prop:Irred_implies_Indecomp} using Maschke's Theorem and its converse.

\begin{proposition}\label{Ch1:Prop:Indecomp_implies_Irred_iff_Comp_Red}
    \letgv. Let $K$ be a field. All indecomposable, finite-dimensional representations of $G$ over $K$ are irreducible if and only if every finite-dimensional representation of $G$ over $K$ is completely reducible.
\end{proposition}
\begin{proof}
    \hfill
    \begin{description}
...
Combining Theorems~\ref{SP:Thm:Maschke} and \ref{Ch1:Thm:Maschke_Converse} gives us the promised partial converse of Proposition \ref{Ch1:Prop:Irred_implies_Indecomp}:
\begin{corollary}
